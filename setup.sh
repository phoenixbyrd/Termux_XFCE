...
#Set Timezone
timezone=$(getprop persist.sys.timezone)
proot-distro login debian --shared-tmp -- env DISPLAY=:1 rm /etc/localtime
proot-distro login debian --shared-tmp -- env DISPLAY=:1 cp /usr/share/zoneinfo/$timezone /etc/localtime

#Set Display in Proot .bashrc
echo "export DISPLAY=:1" >> ../usr/var/lib/proot-distro/installed-rootfs/debian/home/$username/.bashrc
...
DISPLAY=:1 dbus-launch --exit-with-session glxfce &
...
#Install Webcord
proot-distro login --user $varname debian --shared-tmp -- env DISPLAY=:1 wget https://github.com/SpacingBat3/WebCord/releases/download/v4.2.0/webcord_4.2.0_arm64.deb
proot-distro login --user $varname debian --shared-tmp -- env DISPLAY=:1 sudo -S apt install ./webcord_4.2.0_arm64.deb -y
proot-distro login --user $varname debian --shared-tmp -- env DISPLAY=:1 rm webcord_4.2.0_arm64.deb

echo "[Desktop Entry]
Name=Discord
Comment=A Discord and Fosscord client made with the Electron API.
GenericName=Internet Messenger
Exec=proot-distro login debian --user $username --shared-tmp -- env DISPLAY=:1 webcord --no-sandbox
Icon=discord
Type=Application
StartupNotify=true
